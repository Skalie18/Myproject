...
public partial class queueMonitors_MANUAL_FDR_RECEIVECBCREPORT_REQ : System.Web.UI.Page
{
    ...
    {
        var x = Convert.ToDateTime(DateTime.Now.ToString("O"));
        var dir = new DirectoryInfo(@"D:\SARS\PREPROCESSCBC\");
        var files = dir.GetFiles("*.CBCIN");

        foreach (var fileInfo in files)
        {
            var doneFile = Path.Combine(dir.ToString(), "PROD_DONE", Path.GetFileName(fileInfo.FullName));
            ReceiveReport(fileInfo.FullName);
            File.WriteAllText(doneFile, File.ReadAllText(fileInfo.FullName));
        ...
        FDRService.CbcXML.CTSSenderFileMetadata.CTSSenderFileMetadataType metaDataObject, FDRService.CbcXML.CTSSenderFileMetadata.CTSSenderFileMetadataType newCtsSenderFileMetadataType, bool fileRejected, string uid)
    {
        ...
        Sars.Systems.Compression.FileCompression.ZipDirectoryContents(outgoingStatusDirectoryPath, fullZipFileName, FileCompression.CompressionMethod.CompressSlowAndProper);
